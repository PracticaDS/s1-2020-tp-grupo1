...
class ExampleUnitTest {
    ...
    private fun encryptSomethingAndDecriptItB(text : String): String {
        ...
        val textEncriptado : String = encrypt(text)!!
        val textDesencriptado : String = decrypt(textEncriptado!!)!!
    ...
    private fun encryptSomethingAndDecriptIt(text : String): String {
        val plaintext: ByteArray = text.toByteArray()
        val keygen = KeyGenerator.getInstance("AES")
        keygen.init(256)
        val key: SecretKey = keygen.generateKey()
        //val key2: SecretKey = keygen.generateKey()
        ...
        val iv: ByteArray = cipher.iv // Asi esta en la documentacion pero no se usa.
        val ivspec = IvParameterSpec(iv)

        cipher.init(Cipher.DECRYPT_MODE, key, ivspec)
        val ciphertextDecripted: ByteArray = cipher.doFinal(ciphertext)

        return ciphertextDecripted.toString()
    }
}
